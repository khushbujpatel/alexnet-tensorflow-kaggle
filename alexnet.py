...
class AlexNet():
    """
    Class AlexNet (train, retrain, model def, inference)
    """
    def model(self, features, net_data=None, keep_prob=1.0, features_extract=False):
        mu = 0
        sigma = 1e-2
    ...
    def train(self, X_train, y_train, net_weights=None, learning_rate=0.001, epochs=10, batch_size=128, save_graph="alexnet.pb"):
        """
        train/retrains model for given train set

        Retrains if net_weights arg is provided
        """
        net_data = None
        ...
        fc7 = self.model(features, net_data=net_data, features_extract=True, keep_prob=0.5)
        if net_data:
    ...
    def inference(self, fname, model, labels):
        """
        Args
...
def main():
    ...
    if args.train and (not os.path.exists(args.train_p) or not os.path.exists(args.test_p)):
        raise Exception("Dataset information (train.p or test.p) is missing.")
    if args.infer and not os.path.exists(args.infer):
        raise Exception("Given image {} for inferencing does not exist".format(args.infer))
    if not os.path.exists(args.labels):
        raise Exception("Labels file {} does not exist".format(args.labels))
    ...
    if args.train:
        alexnet.train(X_train, y_train, net_weights=args.net_weights, epochs=args.epochs,
                      batch_size=args.batch_size, learning_rate=args.learning_rate, save_graph=args.save_graph)

    if args.infer:
        alexnet.inference(args.infer, args.save_graph, args.labels)
